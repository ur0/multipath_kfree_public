...
kern_return_t remountRootAsRW() {
  ...
  if (getuid() + getgid() + kernel_base == 0) {
    printf("Mounting the root filesystem requires kernel priviliges, we do not "
           "seem to have such.\n");
  ...
  printf("Getting the device virtual node for the root partition device at "
         "%s...\n",
         devpath);
  // uint64_t devVnode = getVnodeAtPath(devpath);

  printf(
      "Assuming that the type of the root filesystem is Apple Filesystem...\n");

  printf("Patching the device node's flags in the kernel...\n");
  // writeKernelMemory(devVnode + off_v_specflags, sizeof(int), 0); // clear dev
  // vnode’s v_specflags
  ...
  // readKernelMemory(newMPMount + off_mnt_data, sizeof(uint64_t),
  // &newMPMountData);
  printf("Our mountdata is: %#llx\n", newMPMountData);

  /* 3. Modify root mount’s flag and remount */
  printf("Modifying the rootfs mount flag so we will bypass the security "
         "checks...\n");
  ...
  // readKernelMemory(rootMount + off_mnt_flag, sizeof(uint32_t),
  // &rootMountFlag);

  printf("The rootfs mount flag is now: %#x\n", rootMountFlag);
  // writeKernel(rootMount + off_mnt_flag, sizeof(uint64_t) ,rootMountFlag & ~ (
  // MNT_NOSUID | MNT_RDONLY | MNT_ROOTFS));
  ...
  printf("Fixing up the final mount's extents data to prevent kernel panics "
         "and bypass extents overflow checks...\n");
  // writeKernelMemory(rootMount + off_mnt_data, sizeof(newMPMountData),
  // newMPMountData);
