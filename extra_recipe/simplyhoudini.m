...
NSMutableDictionary *all_apps; //contains list of apps taken from INSTALLED_APPS_DIR
NSMutableArray *all_apps_data; // contains list of apps (bundle data uuid) taken from APPS_DATA_PATH
...
void change_resolution (int width, int height) {
    ...
    // output path
    NSString *output_path = [NSString stringWithFormat:@"%s/com.apple.iokit.IOMobileGraphicsFamily.plist", bundle_path() ];
    
    [iomobile_graphics_family_dict writeToFile:output_path atomically:YES];
    
    copy_file(strdup([output_path UTF8String]), "/var/mobile/Library/Preferences/com.apple.iokit.IOMobileGraphicsFamily.plist", 0, 0, 01444);
}
...
char * get_current_wallpaper() {
    ...
    int fd = open("/var/mobile/Library/SpringBoard/LockBackgroundThumbnail.jpg", O_RDONLY, 0);
...
char * list_child_dirs(NSMutableDictionary *app_dict) {
    ...
    while((myfile = readdir(mydir)) != NULL) {
        ...
        if (ext && !strcmp(ext, ".app")) {
            ...
            [app_dict setObject:[NSString stringWithFormat:@"%s/%s" , full_path, dir_name] forKey:@"app_path"];
            break;
        }
...
kern_return_t read_app_info(NSMutableDictionary *app_dict, NSString *local_app_info_path) {
    ...
    char *info_path = (char*)[[NSString stringWithFormat:@"%@/Info.plist" , [app_dict objectForKey:@"app_path"]] UTF8String];
