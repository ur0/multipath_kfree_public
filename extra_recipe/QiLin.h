...
//             actually exported the set*Reporter functions (formerly
//             ErrorHandler.. etc - "Reporter" is more accurate, because they
//             allow you to propagate messages to a GUI.
//
...
pid_t execCommand(char *Cmd, char *Arg1, char *Arg2, char *Arg3, char *Arg4,
                  char *Arg5, int Flags);
int execCommandAndWait(char *Cmd, char *Arg1, char *Arg2, char *Arg3,
                       char *Arg4, char *Arg5);

int setTFP0AsHostSpecialPort4(void);

// 1/17/18 - This is super useful
int spawnAndPlatformize(char *AmfidebPath, char *Arg1, char *Arg2, char *Arg3,
                        char *Arg4, char *Arg5);
...
char *cdHashOfFile(char *fileName,
                   int Algorithm); // Calculate CDHash of a given Mach-O (for
                                   // messing with AMFI)
...
// work on processes you are the owner of (or all, if root) - this is
// intentional
...
int makeLaunchdPlist(char *PlistName, char *Program, char *ProgramArguments,
                     char *StandardOutputPath, char *StandardErrorPath,
                     int RunAtLoad);
...
int spawnJailbreakServer(char *Name, mach_port_t TFP0,
                         mach_port_name_t NameInTarget);
...
// Utility functions you probably won't need unless you want to do your own
// debugging
...
uint64_t findMyProcStructInKernelMemory(
    void); // For other advanced uses I haven't provided already
